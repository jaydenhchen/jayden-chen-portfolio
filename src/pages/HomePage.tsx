        ...
        <>
          <section className="hero page-shell" aria-labelledby="hero-title">
            ...
            <Reveal className="hero-media-wrap">
              ...
              >
                <MediaFrame
                  asset={featuredProject.heroMedia}
                  variant="hero"
                  autoplayPreview={featuredProject.heroMedia.kind === 'video'}
                  hoverAudio={featuredProject.heroMedia.kind === 'video'}
                  loading="eager"
                />
              </Link>
            </Reveal>
            ...
            <div className="work-rows">
              <div className="work-row">
                <Reveal className="work-row-heading">
                  <div>
                    <Link className="work-row-title" to="/cad">
                      <h3 id="engineering-work-title">Engineering</h3>
                    </Link>
                    <p>Fusion 360 and hands-on work</p>
                  </div>
                </Reveal>
                <div className="project-grid" aria-labelledby="engineering-work-title">
                  {homeEngineeringProjects.length > 0 ? (
                    homeEngineeringProjects.map((project) => <ProjectCard key={project.slug} project={project} />)
                  ) : (
                    <p className="empty-state">Engineering projects will appear here.</p>
                  )}
                </div>
              </div>
              <div className="work-row">
                <Reveal className="work-row-heading">
                  <div>
                    <Link className="work-row-title" to="/cgi">
                      <h3 id="cgi-work-title">Blender</h3>
                    </Link>
                    <p>Design, animate, render, composite</p>
                  </div>
                </Reveal>
                <div className="project-grid" aria-labelledby="cgi-work-title">
                  {homeCgiProjects.length > 0 ? (
                    homeCgiProjects.map((project) => <ProjectCard key={project.slug} project={project} />)
                  ) : (
                    <p className="empty-state">CGI projects will appear here.</p>
                  )}
                </div>
              </div>
            </div>
            ...
            <Reveal className="contact-panel-reveal">
              <div className="contact-panel">
                ...
                <div className="contact-action text-reveal">
                  <p>There's only so much space in a portfolio. Connect to learn more.</p>
                  {siteProfile.email ? (
                    <a className="button button-primary" href={`mailto:${siteProfile.email}`}>
                      CONNECT VIA EMAIL
                    </a>
                  ) : (
